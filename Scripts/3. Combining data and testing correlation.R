...
activecentres2023 <- fx.total(HN2023)
activecentres2022 <- fx.total(HN2022)
...
par(mfrow=c(1,2))
break23 <- c(0, 0.08, 0.0035)
break22 <- c(0,0.06,0.0035)
...
valuescentres2023 <- valuescentres2023 %>%
  mutate(D.sum.category = case_when(
    ...
    D.sum > 0.008 & D.sum <= 0.015 ~ "0.008-0.015",
    TRUE ~ ">0.015"),
         Year = "2023")%>%
...
if (!is.null(boot_results)) {
  # Check if some bootstrap runs failed (contain NA values)
